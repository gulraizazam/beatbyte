...
class FrontendController extends Controller
{
    ...
    {
      $allstems = Stem::all();
      $allbeats = Beat::join('users','users.id','=','beats.user_id')->select('beats.name as beatname','beats.basic_price','users.name as username','beats.is_free','beats.image','beats.file','beats.id','beats.premium_price','beats.unlimited_price')->paginate(10);
      $allgnerations = Generation::all();
      $allmoods = Mood::all();
    ...
    public function SearchData(Request $request)
    {
      if(is_null($request->searcdata)){
         ...
         $allbeats = DB::table('beats')->join('users','beats.user_id','=','users.id')->select('beats.name as beatname','beats.file','beats.image','beats.basic_price','beats.is_free','beats.id','users.name')->paginate(10);
      }else{
    ...
    public function Search(Request $request)
    {
     ...
     $allbeats = DB::table('beats')->join('users','beats.user_id','=','users.id')->select('beats.name as beatname','beats.file','beats.image','beats.basic_price','beats.is_free','beats.id','users.name')->where('beats.name', 'like',$request->search)->paginate(10);
      }
