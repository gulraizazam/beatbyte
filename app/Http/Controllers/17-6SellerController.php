...
class SellerController extends Controller
{
    public function index()
      ...
      if($request->file('brand')){
          $uniqueid=uniqid();
          $original_name=$request->file('brand')->getClientOriginalName();

          $size=$request->file('brand')->getSize();

          $extension=$request->file('brand')->getClientOriginalExtension();
          
          $filename=Carbon::now()->format('Ymd').'_'.$uniqueid.'.'.$extension;

          $brandpath=url('/storage/app/uploads/images/'.$filename);
          $path=$request->file('brand')->storeAs('/uploads/images',$filename);
         
        $addbeat->brand_logo = $brandpath;
      }
