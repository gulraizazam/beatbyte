...
class PaymentController extends Controller
{
	 		...
	 		foreach(Cart::getContent() as $item ){
               ...
               $uniqueOrderKey = Session::get('orderkey');
                try{
                    ...
                    Mail::send('frontend.emailview',['pagelink'=>'http://dev.music-site/mysongs', 'unique'=>$uniqueOrderKey],function($message) {
    ...
    public function PackegePayment(RequestVar $request,$id)
    {
        ...
        $items[] = (new Item())
                ...
                ->setCurrency('USD')
                ->setQuantity(1)
                ->setPrice($checkoutpkg->packege_price);
       $itemList = new ItemList();
        $itemList->setItems($items);
        ...
        $amount = new Amount();
        $amount->setCurrency('USD')
            ->setTotal($checkoutpkg->packege_price);
    ...
    public function PaymentStatus()
    {
        ...
        if ($result->getState() == 'approved') {
            ...
            $packegepayment->save();
             $pageurl = 'http://dev.music-site/completesignup?key='.Session::get('key');
                try{
