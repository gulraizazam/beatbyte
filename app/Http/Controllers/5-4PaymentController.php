...
class PaymentController extends Controller
{
    ...
    public function payWithpaypal(RequestVar $request)
    {
       ...
       Session::put('email',$request->email);
        ...
        $getuser = Accountsetting::join('users','users.id','=','accountsettings.user_id')->join('songs','users.id','=','songs.user_id')->where('songs.id',$itemid)->first();
        if (!$getuser) {
             return redirect('cart')->with('error','User not exist');
        }
        $clientid = $getuser->PAYPAL_LIVE_API_CLIENT_ID;
        $clientsecret = $getuser->PAYPAL_LIVE_API_SECRET;
    ...
    public function UpgradePackege($id)
    {
