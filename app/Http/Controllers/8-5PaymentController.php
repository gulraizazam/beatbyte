...
class PaymentController extends Controller
{
   ...
   public function StemPayment(RequestVar $request)
   {
        Validator::make($request->all(), [
        'firstname' => 'required',
        'lastname' => 'required',
       ...
       Session::put('email',$request->email);
        ...
        $redirectURLs = new RedirectUrls();
        $redirectURLs->setReturnUrl(URL::to('stemstatus'))
            ->setCancelUrl(URL::to('stemstatus'));
   ...
   public function getStemPaymentStatus()
    {
        $paymentId = Session::get('paypalPaymentId');
        ...
        if ($result->getState() == 'approved') {
            ...
            foreach(Cart::getContent() as $item ){
               ...
               $uniqueOrderKey = Session::get('orderkey');
                ...
                try{
                    $zipfile = Stem::find($item->attributes->stem_id);
                    Mail::send(array(),array(),function($message) use($zipfile) {
                        $message->to(Session::get('email'), "Test")
                        ->from('no-reply@beatbyte.co')
                        ->subject("Download Link")
