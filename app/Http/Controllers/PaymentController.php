    ...
    public function payWithpaypal(RequestVar $request)
    {
       ...
       Session::put('email',$request->email);
        ...
        if (!$getbeatsuser && !$getsongsuser && !$getstemsuser) {
            Session::flash('error', 'Seller not exist!'); 
    ...
    public function getPaymentStatus()
    {
        ...
        if ($result->getState() == 'approved') {
            ...
            foreach(Cart::getContent() as $item ){
                ...
                $getbeat = Beat::find($item->id);
                if($item->price == $getbeat->exclusive_price){
                     $orderItem->beat_type = 'Exclusive';
                }
                if($item->price == $getbeat->unlimited_price){
                     $orderItem->beat_type = 'Unlimited';
                }
                if($item->price == $getbeat->basic_price){
                     $orderItem->beat_type = 'Basic';
                }
                 if($item->price == $getbeat->premium_price){
                     $orderItem->beat_type = 'Premium';
                }
                $orderItem->save();
            ...
            return Redirect::to('mysongs/'.$uniqueOrderKey)->with('success','Your payment was successful.A Download Link Has Been Sent To Your Email. Thank you.');
        }
